...
#[kube(group = "basa62.com", version = "v1", kind = "Foo", namespaced)]
...
const FOO_NAME: &str = "foos.basa62.com";
...
async fn main() -> Result<()> {
    ...
    let _ = crds.delete(FOO_NAME, &dp).await.map(|res| {
        res.map_left(|o| {
            info!(
                "Deleting {}: ({:?})",
                o.name_any(),
    ...
    match crds.delete(FOO_NAME, &dp).await? {
        Left(o) => {
            info!(
